...
import Models.PhongKhachDat_CustomTable_PhongTrong;
import Helpers.DataValidator;
import Models.PhongKhachDat;
import Helpers.DatabaseHelper;
import Controllers.PhongKhachDat_Controller;
import Controllers.PhongKhachDat_KhachHang_Controller;
import Controllers.PhongKhachDat_PhongTrong_Controller;
...
import Models.PhongKhachDat_CustomTable;
import Models.PhongKhachDat_CustomTable_KhachHang;
...
public class QuanLyDatPhong_Panel extends javax.swing.JPanel {
    ...
    static QLPhongController qlPhongDao= new QLPhongController();
    int selectedRow = 0;
    ...
    public String SinhMaTuDong(){
        ...
        if(list.size()== 0){
        ...
        else if(list.size()==1 && Integer.parseInt(list.get(0).getMaDatPhong().toString().substring(2, 5))==1){
            coso=2;
        }
        else if(list.size()==1 && Integer.parseInt(list.get(0).getMaDatPhong().toString().substring(2, 5))>1){
            coso=1;
            ...
            for(i = 0; i < list.size()-1; i++){
                ...
                if(Integer.parseInt(list.get(i+1).getMaDatPhong().toString().substring(2, 5))-Integer.parseInt(list.get(i).getMaDatPhong().toString().substring(2, 5))>1){
                    coso=Integer.parseInt(list.get(i).getMaDatPhong().toString().substring(2, 5))+1;
                    break;
                }
            }
            if(i == list.size()-1 ){
                coso=Integer.parseInt(list.get(list.size()-1).getMaDatPhong().toString().substring(2, 5))+1;
            }
        }
        if(coso<10)
            return "HD00"+coso;
...
//        loadCombobox();
    ...
    private void initComponents() {
        ...
        btnDelete.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Icons/edit.png"))); // NOI18N
        ...
        btnReset.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Icons/find.png"))); // NOI18N
        ...
        btnLocLoaiPhong.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Icons/find.png"))); // NOI18N
        ...
        jLabel14.setText("Danh sách khách thuê:");
        ...
        jLabel19.setText("Tìm khách thuê:");
                ...
                if (xn == JOptionPane.YES_OPTION) {
                    try {
                       ...
                       for(PhongKhachDat pk : list){
                        ...
                        if(pkd.getTinhTrang().equals("Đã thanh toán")){
                            JOptionPane.showMessageDialog(null, "Đơn đặt này đã thanh toán, không cho xóa", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                            ClearTxt();
                            return;
                       }
